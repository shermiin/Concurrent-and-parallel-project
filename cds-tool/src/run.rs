...
pub fn run(
    ...
    let url = try!(hyper::Url::parse(&url).chain_err(|| format!("unable to parse url {}", url)));
    ...
    let mut response = try!(client
        .post(url)
        .body(hyper::client::Body::SizedBody(
            &mut req_body.as_bytes(),
            req_body.len() as u64
        ))
        .send()
